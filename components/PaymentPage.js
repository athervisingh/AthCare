...
const PaymentPage = ({ username }) => {
  ...
  const [paymentform, setPaymentform] = useState({ name:`${username}`,message:"With this gift, I’m helping to give animals the care they deserve. ❤️",amount:""})
...
/>
  ...
  <div className='absolute left-1/2 transform -translate-x-1/2 text-black bg-green-100/30 p-3 rounded-lg shadow-[4px_4px_6px_rgba(255,255,255,0.6)]'>
    ...
    <img src="image/protect.png" className='max-md:w-16 bg-white rounded-full opacity-45 w-40 mx-auto mt-4' alt="" />
  </div>

         {currentUser.coverPic===""? <img src="image/top.webp" className="w-full h-[60vh]" alt="top"/>: <img src={currentUser.coverPic} className="w-full h-[60vh]" alt="top" />}
        {currentUser.profilePic===""?<img src='image/animal.webp' alt="" className='w-44 absolute left-1/2 transform -translate-x-1/2 top-[90%] border-2 shadow-custom-dark border-white rounded-lg shadow-black max-md:w-32 max-sm:w-24' /> :  <img src={currentUser.profilePic} alt="animal dp" className='w-44 absolute left-1/2 transform -translate-x-1/2 top-[90%] border-2 shadow-custom-dark border-white rounded-lg shadow-black max-md:w-32 max-sm:w-24' /> }
</div>
  ...
  <img
    ...
    className="w-9 h-9 rounded-full mb-2 md:mb-0 md:mr-2"
    alt=""
  />
